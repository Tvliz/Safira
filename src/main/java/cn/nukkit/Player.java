...
import cn.nukkit.network.player.PlayerPacketHandler;
...
public class Player extends EntityHuman implements CommandSender, InventoryHolder, ChunkLoader, IPlayer
{
	...
	{
		if (this.spawned && player.spawned && this.isAlive() && player.isAlive() && player.getLevel() == this.level
		...
		for (Entity entity : this.level.getNearbyEntities(this.boundingBox.grow(1, 0.5, 1), this))
		{
			...
			if (entity instanceof EntityArrow && ((EntityArrow) entity).hadCollision)
			{
				...
				this.server.getPluginManager()
					.callEvent(ev = new InventoryPickupArrowEvent(this.inventory, (EntityArrow) entity));
				if (ev.isCancelled())
				{
					continue;
				}
			...
			} else if (entity instanceof EntityItem)
			{
				if (((EntityItem) entity).getPickupDelay() <= 0)
				{
					...
					if (item != null)
					{
						...
						this.server.getPluginManager()
							.callEvent(ev = new InventoryPickupItemEvent(this.inventory, (EntityItem) entity));
						if (ev.isCancelled())
						{
							continue;
						}
	...
	protected void processMovement(int tickDiff)
	{
				...
				if (!this.isSleeping())
				{
					if (diff > 0.125)
					{
						...
						if (!ev.isCancelled())
						{
							...
							if (revert)
							{
								this.server.getLogger().warning(this.getServer().getLanguage()
									.translateString("nukkit.player.invalidMove", this.getName()));
							}
						}
					}
				}
		...
		if (!revert
		    && (Math.pow(this.lastX - to.x, 2) + Math.pow(this.lastY - to.y, 2)
	...
	public boolean setMotion(Vector3 motion)
	{
		if (super.setMotion(motion))
		{
			...
			{
				// todo: check this
				this.startAirTicks = (int) ((-(Math
					.log(this.getGravity() / (this.getGravity() + this.getDrag() * this.motionY))) / this.getDrag())
				                            * 2 + 5);
			}
				...
				} else
				{
					    ...
					    && !this.getDataPropertyBoolean(DATA_NO_AI))
					{
						double expectedVelocity = (-this.getGravity()) / ((double) this.getDrag())
						                          - ((-this.getGravity()) / ((double) this.getDrag())) * Math.exp(
							-((double) this.getDrag()) * ((double) (this.inAirTicks - this.startAirTicks)));
	...
	public void checkNetwork()
	{
		...
		if (!this.batchedPackets.isEmpty())
		{
			...
			{
				this.server.batchPackets(new Player[] { this },
	...
	protected void processLogin()
	{
		...
		{
			this.spawnPosition = new Position(this.namedTag.getInt("SpawnX"), this.namedTag.getInt("SpawnY"),
		...
		updateAttributesPacket.entries = new Attribute[] {
			Attribute.getAttribute(Attribute.MAX_HEALTH).setMaxValue(this.getMaxHealth())
		...
		this.server.getLogger()
			.info(this.getServer().getLanguage().translateString("nukkit.player.logIn", TextFormat.AQUA + this.getUsername() + TextFormat.WHITE,
				this.ip,
	...
	public void handleDataPacket(DataPacket packet)
	{
		...
		if ((handler = PlayerPacketHandler.getPacketFromPid(packet.pid())) != null)
		{
			...
			{
				handler.getClass()
					.getMethod("handlePacket", Player.class, DataPacket.class)
					.invoke(handler, this, packet);

			}
			catch (Exception exception)
			{
				this.getServer().getLogger().error("Could not invoke PacketHandler.handlePacket: ", exception);
			}
		...
		switch (packet.pid())
		{
			case ProtocolInfo.LOGIN_PACKET:
				break; // at cn.nukkit.network.player.handler.LoginPacketHandler
			case ProtocolInfo.MOVE_PLAYER_PACKET:

				MovePlayerPacket movePlayerPacket = (MovePlayerPacket) packet;
				Vector3 newPos = new Vector3(movePlayerPacket.x, movePlayerPacket.y - this.getEyeHeight(),
					movePlayerPacket.z
				);
				...
				if (this.teleportPosition != null || this.forceMovement != null
				                                     && (newPos.distanceSquared(this.forceMovement) > 0.1 || revert))
				{
					this.sendPosition(this.teleportPosition == null ? this.forceMovement : this.teleportPosition,
						movePlayerPacket.yaw, movePlayerPacket.pitch
					);
				} else
				{
				...
				if (riding != null)
				{
					...
					{
						riding.setPositionAndRotation(this.temporalVector.setComponents(movePlayerPacket.x,
								movePlayerPacket.y - 1, movePlayerPacket.z
							), (movePlayerPacket.headYaw + 90) % 360,
							0
						);
					}
				}
			...
			case ProtocolInfo.MOB_EQUIPMENT_PACKET:
				...
				if (mobEquipmentPacket.slot == 0x28 || mobEquipmentPacket.slot == 0
				    || mobEquipmentPacket.slot == 255)
				{
				...
				{
					if (mobEquipmentPacket.selectedSlot >= 0
					    && mobEquipmentPacket.selectedSlot < this.inventory.getHotbarSize())
					{
						this.inventory.setHeldItemIndex(mobEquipmentPacket.selectedSlot);
			...
			case ProtocolInfo.USE_ITEM_PACKET:
				...
				if (useItemPacket.face >= 0 && useItemPacket.face <= 5)
				{
					...
					} else if (this.isCreative())
					{
						...
						if (this.level.useItemOn(blockVector, i, useItemPacket.face, useItemPacket.fx,
							useItemPacket.fy, useItemPacket.fz, this
						) != null)
						{
							break;
						}
					...
					} else
					{
						...
						if ((item = this.level.useItemOn(blockVector, item, useItemPacket.face, useItemPacket.fx,
							useItemPacket.fy, useItemPacket.fz, this
						)) != null)
						{
					...
					this.level.sendBlocks(new Player[] { this }, new Block[] { target, block },
						UpdateBlockPacket.FLAG_ALL_PRIORITY
					);
					break;
				} else if (useItemPacket.face == 0xff)
				{
					Vector3 aimPos = (new Vector3((double) useItemPacket.x / 32768, (double) useItemPacket.y / 32768,
						(double) useItemPacket.z / 32768
					)).normalize();
					...
					PlayerInteractEvent playerInteractEvent = new PlayerInteractEvent(this, item, aimPos,
						useItemPacket.face, PlayerInteractEvent.RIGHT_CLICK_AIR
					);
					...
					} else if (item.getId() == Item.SPLASH_POTION)
					{
						...
						EntityPotion bottleEntity = bottle;
						ProjectileLaunchEvent projectileEv = new ProjectileLaunchEvent(bottleEntity);
						this.server.getPluginManager().callEvent(projectileEv);
						if (projectileEv.isCancelled())
						{
			...
			case ProtocolInfo.PLAYER_ACTION_PACKET:
				...
				switch (((PlayerActionPacket) packet).action)
				{
					case PlayerActionPacket.ACTION_START_BREAK:
						...
						PlayerInteractEvent playerInteractEvent = new PlayerInteractEvent(this,
							this.inventory.getItemInHand(), target, ((PlayerActionPacket) packet).face,
					...
					case PlayerActionPacket.ACTION_RELEASE_ITEM:
						if (this.startAction > -1 && this.getDataFlag(Player.DATA_FLAGS, Player.DATA_FLAG_ACTION))
						{
							if (this.inventory.getItemInHand().getId() == Item.BOW)
							{
								...
								{
									entityShootBowEvent.getProjectile().setMotion(entityShootBowEvent
										.getProjectile().getMotion().multiply(entityShootBowEvent.getForce()));
									if (this.isSurvival())
									{
										...
										if (!bow.hasEnchantments()
										    || (infinity = bow
											.getEnchantment(Enchantment.ID_BOW_INFINITY)) == null
										    || infinity.getLevel() <= 0)
										...
										if (!bow.isUnbreakable())
										{
											...
											if (!(durability != null && durability.getLevel() > 0
											      && (100 / (durability.getLevel() + 1)) <= new Random()
												.nextInt(100)))
											{
												...
												{
													this.inventory
														.setItemInHand(new ItemBlock(new BlockAir(), 0, 0));
												} else
												{
									...
									{
										ProjectileLaunchEvent projectev = new ProjectileLaunchEvent(
			...
			case ProtocolInfo.REMOVE_BLOCK_PACKET:
				...
				this.level.sendBlocks(new Player[] { this }, new Block[] { target },
					UpdateBlockPacket.FLAG_ALL_PRIORITY
				);
			...
			case ProtocolInfo.INTERACT_PACKET:
				...
				boolean cancelled = targetEntity instanceof Player && !((boolean) this.server.getConfig("pvp", true));
				...
				if (targetEntity != null && this.isAlive() && targetEntity.isAlive())
				{
					...
					if (targetEntity instanceof EntityItem || targetEntity instanceof EntityArrow)
					{
						...
						this.server.getLogger().warning(this.getServer().getLanguage()
							.translateString("nukkit.player.invalidEntity", this.getName()));
						break;
					}
					...
					EntityDamageByEntityEvent entityDamageByEntityEvent = new EntityDamageByEntityEvent(this,
						targetEntity, EntityDamageEvent.CAUSE_ENTITY_ATTACK, damage
					);
					if (cancelled)
					{
			...
			case ProtocolInfo.ANIMATE_PACKET:
				...
				PlayerAnimationEvent animationEvent = new PlayerAnimationEvent(
					this,
					((AnimatePacket) packet).action
				);
			...
			case ProtocolInfo.ENTITY_EVENT_PACKET:
				...
				if (entityEventPacket.event == EntityEventPacket.USE_ITEM)
				{ // Eating
			...
			case ProtocolInfo.TEXT_PACKET:
				...
				{
					textPacket.message = this.removeFormat ? TextFormat.clean(textPacket.message)
						: textPacket.message;
					for (String msg : textPacket.message.split("\n"))
					{
						if (!msg.trim().isEmpty() && msg.length() <= 255 && this.messageCounter-- > 0)
						{
							if (msg.startsWith("/"))
							{ // Command
								PlayerCommandPreprocessEvent commandPreprocessEvent = new PlayerCommandPreprocessEvent(
									this, msg);
								...
								this.server.dispatchCommand(
									commandPreprocessEvent.getPlayer(),
			...
			case ProtocolInfo.CRAFTING_EVENT_PACKET:
				...
				if (this.craftingType == CRAFTING_ANVIL)
				{
					...
					if (anvilInventory == null)
					{
						...
						if (anvilInventory == null)
						{ // If it'sf _still_ null, then the player doesn't have a valid
							// anvil window, cannot proceed.
							this.getServer().getLogger()
								.debug("Couldn't find an anvil window for " + this.getName() + ", exiting");
							this.inventory.sendContents(this);
							break;
						}
					}
					...
					if (recipe == null)
					{
						...
						{
							this.getServer().getLogger()
								.debug(this.getName() + " failed to rename an item in an anvil");
							this.inventory.sendContents(this);
						}
					} else
					{
				...
				{
					Recipe[] recipes = getServer().getCraftingManager()
						.getRecipesByResult(craftingEventPacket.output[0]);
					...
					if (recipe != null)
					{
						CraftItemEvent craftItemEvent = new CraftItemEvent(this,
							ingredientz.toArray(Item[]::new), recipe
						);
				...
				} else
				{
					...
					if (recipe instanceof ShapedRecipe)
					{
						...
						for (int x = 0 ; x < 3 - offsetX && canCraft ; ++x)
						{
							for (int y = 0 ; y < 3 - offsetY ; ++y)
							{
								...
								if (item.getCount() > 0)
								{
									if (ingredient == null || !ingredient.deepEquals(item, ingredient.hasMeta(),
					...
					{
						this.server.getLogger()
							.debug("Unmatched recipe " + recipe.getId() + " from player " + this.getName()
							       + ": expected " + recipe.getResult() + ", got " + result + ", using: "
					...
					if (!canCraft)
					{
						this.server.getLogger()
							.debug("Unmatched recipe " + recipe.getId() + " from player " + this.getName()
							       + ": client does not have enough items, using: "
							       + Arrays.asList(ingredients));
						this.inventory.sendContents(this);
						break;
					}
					...
					this.server.getPluginManager()
						.callEvent(craftItemEvent = new CraftItemEvent(this, ingredients, recipe));
			...
			case ProtocolInfo.CONTAINER_SET_SLOT_PACKET:
				...
				{ // Our inventory
					...
					transaction = new BaseTransaction(this.inventory, containerSetSlotPacket.slot,
						this.inventory.getItem(containerSetSlotPacket.slot), containerSetSlotPacket.item
					);
				...
				{ // Our
					...
					transaction = new BaseTransaction(this.inventory,
						containerSetSlotPacket.slot + this.inventory.getSize(),
				...
				} else if (this.windowIndex.containsKey(containerSetSlotPacket.windowid))
				{
					...
					{
						((EnchantInventory) inv).onEnchant(this, inv.getItem(containerSetSlotPacket.slot),
							containerSetSlotPacket.item
						);
					}

					transaction = new BaseTransaction(inv, containerSetSlotPacket.slot,
						inv.getItem(containerSetSlotPacket.slot), containerSetSlotPacket.item
					);
				} else
				{
					break;
				...
				if (transaction.getSourceItem().deepEquals(transaction.getTargetItem())
				    && transaction.getTargetItem().getCount() == transaction.getSourceItem().getCount())
				{ // No
					// changes!
				...
				if (t instanceof BlockEntitySign)
				{
					...
					} else
					{
						SignChangeEvent signChangeEvent = new SignChangeEvent(t.getBlock(), this, new String[] {
							this.removeFormat ? TextFormat.clean(nbt.getString("Text1"))
								: nbt.getString("Text1"),
						...
						if (!t.namedTag.contains("Creator") || !Objects.equals(
							this.getUniqueId().toString(),
							t.namedTag.getString("Creator")
						))
						{
						...
						{
							((BlockEntitySign) t).setText(signChangeEvent.getLine(0), signChangeEvent.getLine(1),
			...
			case ProtocolInfo.ITEM_FRAME_DROP_ITEM_PACKET:
				...
				Vector3 vector3 = this.temporalVector.setComponents(itemFrameDropItemPacket.x,
					itemFrameDropItemPacket.y, itemFrameDropItemPacket.z
				);
				...
				if (itemFrame != null)
				{
					...
					ItemFrameDropItemEvent itemFrameDropItemEvent = new ItemFrameDropItemEvent(this, block,
						itemFrame, itemDrop
					);
					...
					if (!itemFrameDropItemEvent.isCancelled())
					{
						...
						{
							vector3 = this.temporalVector.setComponents(itemFrame.x + 0.5, itemFrame.y,
								itemFrame.z + 0.5
							);
	...
	private EntityShootBowEvent getEntityShootBowEvent(
		Item bow,
		CompoundTag nbt
	)
	{
	...
	)
	{
		if (this.connected && !this.closed)
		{
			...
			if (ev != null && !Objects.equals(this.getUsername(), "") && this.spawned
			...
			this.spawned = false;
			this.server.getLogger()
				.info(this.getServer().getLanguage().translateString("nukkit.player.logOut", TextFormat.AQUA + (this.getName() == null ? "" : this.getName()) + TextFormat.WHITE,
					this.ip,
					String.valueOf(this.port),
	...
	public void kill()
	{
		...
		this.server.getPluginManager().callEvent(ev = new PlayerDeathEvent(this, this.getDrops(),
			new TranslationContainer(message, params.toArray(String[]::new)), this.getExperienceLevel()
		));
	...
	public void setHealth(float health)
	{
		...
		Attribute attr = Attribute.getAttribute(Attribute.MAX_HEALTH).setMaxValue(this.getMaxHealth())
	...
	public void attack(EntityDamageEvent source)
	{
		...
		if (source instanceof EntityDamageByEntityEvent)
		{
			...
			if (!damager.onGround)
			{
				...
				for (int i = 0 ; i < 5 ; i++)
				{
					CriticalParticle par = new CriticalParticle(new Vector3(this.x + (double) random.nextRange(-15, 15) / 10,
						this.y + (double) random.nextRange(0, 20) / 10, this.z + (double) random.nextRange(-15, 15) / 10
					));
	...
	public void onChunkChanged(FullChunk chunk)
	{
		this.loadQueue.put(
			...
			Math.abs(((int) this.x >> 4) - chunk.getX()) + Math.abs(((int) this.z >> 4) - chunk.getZ())
		);
	}
