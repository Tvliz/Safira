...
public class Level implements ChunkManager, Metadatable {
    ...
    private final Map<Long, Map<Long, SetEntityMotionPacket.Entry>> motionToSend = new HashMap<>();

    private final Map<Long, Map<Long, MoveEntityPacket>> moveToSend = new HashMap<>();

    private final Map<Long, Map<Long, MovePlayerPacket>> playerMoveToSend = new HashMap<>();
    ...
    private final PriorityQueue<PriorityObject> updateQueue;
        ...
        if (optimizeRebuilds) {
            ...
            for (Vector3 b : blocks) {
                ...
                if (b instanceof Block) {
                    ...
                    list.add(new UpdateBlockPacket.Entry((int) b.x, (int) b.z, (int) b.y,
    ...
    public int getFullLight(Vector3 pos) {
        ...
        if (chunk != null) {
            ...
            if (level < 15) {
                level = Math.max(
                        chunk.getBlockLight((int) pos.x & 0x0f, (int) pos.y & 0x7f, (int) pos.z & 0x0f),
                        level
                );
                // todo: check this
            }
